...
- **Task 017: 관리자/클라이언트 역할 분리 및 UI 개선** - 우선순위
...
- **Task 018: 클라이언트 공유 링크 기능 구현**
  - 견적서 공유 토큰 생성 및 관리 시스템 구축
  - 공유 링크 생성 API Route 구현 (`/api/invoices/[id]/share`)
  - 공유 토큰 저장소 구현 (Notion DB 별도 테이블 또는 로컬 KV)
  - 견적서 목록의 각 카드에 "링크 복사" 버튼 추가
  - 클립보드 복사 기능 및 토스트 알림 구현
  - 공유 링크용 공개 페이지 구현 (`/shared/[token]`)
  - 공유 페이지 전용 레이아웃 (인증 불필요, 읽기 전용 견적서 상세 뷰)
  - 공유 링크 만료 기간 설정 (기본 7일)
  - 공유 링크 비활성화/삭제 기능 (관리자 전용)
  - Playwright MCP를 활용한 공유 링크 생성/조회 E2E 테스트

- **Task 019: 다크모드 기능 완성 및 전체 UI 테마 적용**
...
- **Task 020: Phase 5 통합 테스트**
  - Playwright E2E 테스트 스위트 작성 (`tests/e2e/phase5-integration.spec.ts`)
  - 관리자 로그인 후 역할 기반 UI 검증 (관리자 뱃지, 관리 메뉴 표시)
  - 공유 링크 전체 플로우 테스트
    - 관리자가 견적서 목록에서 공유 링크 생성
    - 클립보드 복사 동작 확인
    - 공유 링크로 비인증 사용자 접근 시 견적서 조회 성공
    - 만료된 공유 링크 접근 시 에러 페이지 표시
  - 다크모드 전체 페이지 스크린샷 비교 테스트
    - 라이트 모드 → 다크 모드 전환 후 주요 페이지 시각적 검증
    - 시스템 테마 설정 반영 확인
  - 역할 기반 접근 제어 테스트 (관리자 전용 경로 비인가 접근 차단)
  - 에러 핸들링 및 엣지 케이스 테스트
    - 잘못된 공유 토큰 접근
    - 역할 없는 사용자 로그인 시 기본 동작
    - 네트워크 오류 시 공유 링크 생성 실패 처리
...
**진행 상황**: Phase 1-4 완료 (16/16 Tasks) | Phase 5 예정 (0/4 Tasks)
...
- Phase 5: 고도화 - 역할 분리, 공유 기능, 다크모드 (Task 017-020)
  - Task 017: 관리자/클라이언트 역할 분리 및 UI 개선 - 우선순위
  - Task 018: 클라이언트 공유 링크 기능 구현
  - Task 019: 다크모드 기능 완성 및 전체 UI 테마 적용
  - Task 020: Phase 5 통합 테스트
